function solution(io: NodeIO): void {
  const scr = [io.readInt(), io.readInt(), io.readInt(), io.readInt()];
  const scr2 = [io.readInt(), io.readInt()];
  io.write(
    scr
      .sort((a, b) => b - a)
      .slice(0, 3)
      .reduce((a, b) => a + b, 0) + scr2.sort((a, b) => b - a)[0]
  );
}
...
class NodeIO {
  ...
  private buffer: Buffer = require("fs").readFileSync(
    this.debug ? "./src/stdin" : 0
  );
  private stdout: any[] = [];
  ...
  public readWord(): string {
    ...
    while (true) {
      const char = buffer[this.cursor];
      if (char === 10 || char === 13 || char === 32 || char === undefined)
        break;
      else ++this.cursor;
    }

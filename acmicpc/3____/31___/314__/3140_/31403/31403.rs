...
fn solution(io: &mut RustIO, stdout: &mut String) {
    ...
    let anum = a.parse::<i32>().unwrap();
    let bnum = b.parse::<i32>().unwrap();
    let cnum = c.parse::<i32>().unwrap();

    writeln!(stdout, "{}", anum + bnum - cnum).unwrap();

    // a + b as string
    let apb = format!("{}{}", a, b);
    let apbnum = apb.parse::<i32>().unwrap();
    writeln!(stdout, "{}", apbnum - cnum).unwrap();
}

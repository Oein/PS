...
fn main() {
    ...
    let a = io.read_i32() as usize;

    let d = [
        vec![1, 2, 3, 4, 5, 6, 7, 8, 10, 12, 13],
        vec![1, 3, 5, 6, 7, 8, 9, 12, 13],
        vec![1, 3, 5, 6, 7, 8, 9, 12, 13],
        vec![1, 2, 3, 5, 6, 7, 8, 12, 13],
        vec![1, 3, 5, 6, 7, 8, 12, 13],
        vec![1, 3, 5, 6, 7, 8, 12, 13],
        vec![1, 3, 5, 6, 7, 8, 12, 13],
        vec![1, 3, 5, 6, 7, 8, 12, 13],
        vec![1, 3, 5, 6, 7, 8, 12, 13],
        vec![1, 2, 3, 6, 7, 8, 12, 13],
    ];

    println!("{}", d[a as usize - 1].len());
    for dx in d[a as usize - 1].iter() {
        print!("{} ", (b'A' + dx - 1) as char);
    }
}
